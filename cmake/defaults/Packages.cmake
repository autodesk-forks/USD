...
if (PXR_BUILD_IMAGING)
    ...
    if (PXR_BUILD_OPENIMAGEIO_PLUGIN)
        ...
        set(OIIO_VERSION "${OpenImageIO_VERSION_MAJOR}.${OpenImageIO_VERSION_MINOR}.${OpenImageIO_VERSION_PATCH}")
        add_definitions(-DPXR_OIIO_PLUGIN_ENABLED)
        if (TARGET OpenImageIO::idiff)
            if (CMAKE_BUILD_TYPE)
                string(TOUPPER "${CMAKE_BUILD_TYPE}" UPPER_BUILD_TYPE)
                get_target_property(OIIO_idiff_BINARY OpenImageIO::idiff IMPORTED_LOCATION_${UPPER_BUILD_TYPE})
            else()
                get_target_property(OIIO_idiff_BINARY OpenImageIO::idiff IMPORTED_LOCATION)
            endif()
        endif()
        if (OIIO_idiff_BINARY)
            set(IMAGE_DIFF_TOOL ${OIIO_idiff_BINARY} CACHE STRING "Uses idiff for image diffing")
        elseif (PXR_BUILD_TESTS)
            message(FATAL_ERROR "Unable to find idiff for image diffing tests")
        endif()
    endif()
    # --OpenColorIO
