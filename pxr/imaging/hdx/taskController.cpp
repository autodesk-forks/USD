    ...
    HdxColorCorrectionTaskParams const& params)
{
    ...
    if (!_presentTaskId.IsEmpty()) {
        ...
        if (presentParams.params.surface.srcColorSpace != srcColorSpace ||
            ...
            presentParams.params.surface.preferredDstColorSpace = dstColorSpace;
            dirtyPresentParams = true;
        }
        if (dirtyPresentParams) {
            _delegate.SetParameter(
                _presentTaskId, HdTokens->params, presentParams);
            GetRenderIndex()->GetChangeTracker().MarkTaskDirty(
                _presentTaskId, HdChangeTracker::DirtyParams);
        }
    }
...
void
HdxTaskController::EnablePresentation(HgiPresent2 presentation)
{
    if (_presentTaskId.IsEmpty()) {
        return;
    }
...
void
HdxTaskController::EnableVsync(bool vsync)
{
    if (_presentTaskId.IsEmpty()) {
        return;
    }
...
HdxTaskController::_SetCameraFramingForTasks()
{
    ...
    if (!_presentTaskId.IsEmpty()) {
        ...
        const GfRect2i dstRegion = _framing.IsValid() ?
            GfRect2i({0, 0}, _renderBufferSize[0], _renderBufferSize[1]) :
            _ToRect2i(_viewport);
