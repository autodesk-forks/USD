...
static bool
_GetFragmentBarycentricSupport(
    const HdStResourceRegistrySharedPtr& resourceRegistry)
{
    const HgiCapabilities* capabilities =
        resourceRegistry->GetHgi()->GetCapabilities();
        ...
        if (geomSubsets.empty() || desc.geomStyle == HdMeshGeomStylePoints) {
            ...
            if (rangeInstance.IsFirstInstance()) {
                ...
                if (desc.geomStyle == HdMeshGeomStylePoints) {
                    ...
                    // Line rendering computations rely on barycentric coordinates in the
                    // fragment shader. These are either built-in or can be generated by
                    // the geometry/tessellation shader. When neither of these capabilities
                    // are available, we fall back to drawing line primitives.
                    bool const useLineFallback = !_GetFragmentBarycentricSupport(resourceRegistry) &&
                        (desc.geomStyle == HdMeshGeomStyleEdgeOnly ||
                         desc.geomStyle == HdMeshGeomStyleHullEdgeOnly);

                    if (useLineFallback) {
                        // Use our custom computation to generate line indices.
                        // The output buffer is still named "indices", so the rest
                        // of the pipeline will use it automatically.
                        source = std::make_shared<HdSt_LineIndexComputation>(
                            GetTopology(), GetId());
                    } else {
                                         ...
                                         const SdfPath &materialId)
{
    ...
    } else if ((desc.geomStyle == HdMeshGeomStyleEdgeOnly ||
               desc.geomStyle == HdMeshGeomStyleHullEdgeOnly) &&
            !_GetFragmentBarycentricSupport(resourceRegistry)) {
        primType = PrimitiveType::PRIM_BASIS_CURVES_LINES;
