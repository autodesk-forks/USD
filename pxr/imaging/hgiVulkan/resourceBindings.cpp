    ...
    , _vkDescriptorSet(nullptr)
{
    ...
    // the descriptorSets. But if we have no descriptors at all we will skip
    // creating the pool and descriptors all together. This is still wasteful
    // though, so warn. Better to skip creating the Hgi bindings completely.
    uint32_t totalDescriptorCount = 0;
    for (VkDescriptorPoolSize& pool : poolSizes) {
        totalDescriptorCount += pool.descriptorCount;
        pool.descriptorCount = std::max(pool.descriptorCount, 1u);
    }

    if (totalDescriptorCount <= 0) {
        TF_WARN("Creating an empty resource binding");
