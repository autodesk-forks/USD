...
# Autodesk - updated to v1.2.13 to fix two major vulnerabilities.
...
def InstallOpenImageIO(context, force, buildArgs):
    ...
    with CurrentWorkingDirectory(DownloadURL(OIIO_URL, context, force)):
        ...
        #
        # The fix is to search for OpenEXR 2.3.0 exactly. When
        # OpenEXR is upgraded, we will upgrade (or remove) this patch.
        _findOpenEXR_patch = [
            ("pkg_check_modules(_OPENEXR QUIET OpenEXR>=2.0.0)",
             "pkg_check_modules(_OPENEXR OpenEXR=2.3.0)")]
        if staticOpenEXR:
...
# To fix a bug on Mac when building statically, bump version.
