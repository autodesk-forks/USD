...
///
template <typename PassEncoder, typename = std::enable_if_t<std::is_same_v<PassEncoder, wgpu::RenderPassEncoder> || std::is_same_v<PassEncoder, wgpu::RenderBundleEncoder>>>
class HgiWebGPUBaseGraphicsCmds : public HgiGraphicsCmds
{
public:
    HGIWEBGPU_API
    ...
    void ExecutePrerecordedCmds(HgiCmds* cmds) override {
        TF_CODING_ERROR("ExecutePrerecordedCmds not implemented for Base class");
    }
