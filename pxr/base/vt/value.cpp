...
VtValue::VtValue(VtValueRef ref) { *this = ref.operator VtValue(); }

VtValue &VtValue::operator=(VtValueRef ref) {
...
bool VtValue::IsArrayEditValued() const {
...
VtValue::operator VtValueRef() && { return Ref(); }

VtValue::operator VtValueRef() const & { return Ref(); }
...
bool VtValue::CanComposeOver() const {
  return !_info.GetLiteral() || _info->canComposeOver;
}

bool VtValue::CanTransform() const {
...
emscripten::val VtValue::_GetJsVal() const {
  return _info.GetLiteral() ? _info.Get()->GetJsVal(_storage)
                            : emscripten::val::undefined();
}
#endif // PXR_JS_BINDINGS_SUPPORT_ENABLED
const void *VtValue::_GetHeldObjectPtrForDebugger() const {
